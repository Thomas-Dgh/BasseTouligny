...
class CFF_Utils{
	...
	static function cff_eventdate($original, $date_format, $custom_date, $cff_event_timezone_offset, $cff_timezone) {
		...
		$processed = $original;
		if ( $cff_event_timezone_offset == 'true' ) {
			$processed = CFF_Utils::cff_set_timezone( $original, $cff_timezone );
		}
		//If timezone migration is enabled then remove last 5 characters
		if ( strlen( $processed ) == 24 ) {
			$processed = substr ($processed, 0, -5 );
		}
		$processed = apply_filters( 'cff_event_date', $processed, $original, $cff_event_timezone_offset, $cff_timezone );
		$original  = $processed;
